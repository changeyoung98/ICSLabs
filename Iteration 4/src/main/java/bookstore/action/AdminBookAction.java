...
public class AdminBookAction extends BaseAction {
    ...
    public String allBooksView() {
        ...
        setPageTitle("E-bookstore managing system - book management");
        setViewProfile();
        return SUCCESS;
    }
    ...
    public String addBook() {
        ...
        Validator vd = new Validator(getName(), "booktitle");
        if (!vd.validateNotEmpty()) {
            retJson = vd.getFailureMessage();
            return ERROR;
        }
        vd = new Validator(getPrice(), "price");
        if (!vd.validateNotEmpty() || !vd.validatePattern("^[0-9]+(?:\\.[0-9]{1,2})?$")) {
            retJson = vd.getFailureMessage();
            return ERROR;
        }
        vd = new Validator(getStock(), "stock");
    ...
    public String updateBook() {
        ...
        vd = new Validator(getName(), "booktitle");
        if (!vd.validateNotEmpty()) {
            retJson = vd.getFailureMessage();
            return ERROR;
        }
