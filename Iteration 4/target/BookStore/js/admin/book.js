...
function loadAllBooks() {
    $.get(allBooksAdminAPI, function (data, status) {
        ...
        $('.alert').remove();
        $('h1').after(errorAlertHTML('Fail to load book details！<br>Error：' + error));
    });
}
...
function addBookCallback(data, status, newName, newPrice, newStock) {
    ...
    if (data.result == 'success') {
        $('#addBookStatus').html(shortSuccessText('Add Success！'));
        setTimeout(function () {
        ...
        if (data.msg)
            ...
            $('#addBookStatus').html(shortFailText('Unknown Error'));
    }
}
...
function deleteBookCallback(data, status, id) {
    ...
    if (data.result == 'success') {
        $('#deleteBookStatus').html(shortSuccessText('Delete Success！'));
        setTimeout(function () {
        ...
        if (data.msg)
            ...
            $('#deleteBookStatus').html(shortFailText('Unknown Error'));
    }
}
    ...
    for (i in [0, 1]) {
        bookUploaders[i] = new ss.SimpleUpload({
            ...
            onSubmit: (function (action) {
                return function () {
                    $('#' + action + 'BookUploadTips').html(shortProcessingText('Uploading pictures...'));
                };
            })(actions[i]),
            onSizeError: (function (action) {
                return function() {
                    $('#' + action + 'BookUploadTips').html(shortFailText('The size of the pictures should be less than 2M'));
                };
            })(actions[i]),
            onExtError: (function (action) {
                return function() {
                    $('#' + action + 'BookUploadTips').html(shortFailText('Only JPG、PNG、BMP、GIF avilable'));
                };
            })(actions[i]),
...
$('#updateDialog').on('show.bs.modal', function (event) {
    ...
    $('#updateDialogLabel').text('Book details - NO：' + updateId);
    ...
    $('#bookCategories').html('');
    $('#timeDetailText').html(shortProcessingText('Loading book details...', true));
    ...
    $.get(bookDetailAdminAPI + '?id=' + updateId, function (data, status) {
        ...
        if (categoryCount > 0) {
            ...
            $('#bookCategories').append('No category details');
        }
        $('#timeDetailText').html('<div class="col-md-6"><span class="glyphicon glyphicon-check"></span> Time of creation：' + data.createTime + '</div>  \
            <div class="col-md-6 text-right"><span class="glyphicon glyphicon-edit"></span> Time of modification：' + data.updateTime + '</div>');
        $('#btnUpdateBook').removeAttr('disabled');
    }).fail(function (xhr, status, error) {
        $('#timeDetailText').html(shortFailText('Fail to load category info！Error：' + error, true));
    });
});
...
$('#deleteDialog').on('show.bs.modal', function (event) {
    ...
    $('#deleteText').html('Are you sure to delete book <strong>' + deleteId + '</strong>  <strong>' + escapeHtml(deleteName) + '</strong> 吗？');
