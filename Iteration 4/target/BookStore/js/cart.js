...
function updateTotalPrice() {
    ...
    $('#totalPriceText').html('Total Price：&#65509;' + totalPrice.toFixed(2));
}

function cartUpdateCallback(data, status, id, newQuantity) {
    $('#updateStatus').removeClass('hidden');
    if (data.result == 'success') {
        $('#updateStatus').html(shortSuccessText('Modificaion Success！'));
        setTimeout(function () {
        ...
        if (data.msg)
            ...
            $('#updateStatus').html(shortFailText('Unknown Error'));
    }
}
...
function cartDeleteCallback(data, status, id) {
    ...
    if (data.result == 'success') {
        $('#deleteItemStatus').html(shortSuccessText('Deletion Success！'));
        setTimeout(function () {
        ...
        if (data.msg)
            ...
            $('#deleteItemStatus').html(shortFailText('Unknown Error'));
    }
}
...
function payCallback(data, status) {
    ...
    if (data.result == 'success') {
        $('#payStatus').html(shortSuccessText('Payment Success！'));
        setTimeout(function () {
        ...
        if (data.msg)
            ...
            $('#payStatus').html(shortFailText('Unknown Error'));
    }
}
...
$(document).ready(function () {
    ...
    $.get(cartDetailAPI, function (data, status) {
        ...
        $('.alert').remove();
        $('h1').after(errorAlertHTML('Fail to load shopping cart！<br>Error：' + error));
    });
});
...
$('#deleteDialog').on('show.bs.modal', function (event) {
    ...
    $('#deleteText').html('Are you sure to buy ' + deleteQuantity + ' <strong>' + escapeHtml(deleteBookName) + '</strong> item？');
...
$('#payDialog').on('show.bs.modal', function () {
    ...
    $('#payTotalPrice').text('');
    $('#payStatus').removeClass('hidden');
    $('#payStatus').html(shortProcessingText('Loading shopping cart informtion...'));
    ...
    $.get(cartSummaryAPI, function (data, status) {
        ...
        $('#payStatus').html(shortFailText('Fail to load shopping cart！Error：' + error));
    });
});
