...
function registerCallback(data, status) {
    ...
    if (data.result == 'success') {
        $('#registerStatus').html(shortSuccessText('Register Success！'));
        setTimeout(function () {
        ...
        if (data.msg)
            ...
            $('#registerStatus').html(shortFailText('Unknown Error'));
    }
}
...
function loginCallback(data, status) {
    ...
    if (data.result == 'success') {
        $('#loginStatus').html(shortSuccessText('Login Success！'));
        ...
        $('#loginStatus').html(shortFailText('Unkwown Error'));
}
